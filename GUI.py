...
portname = "COM7"  # Default port name
hexmode = False  # Flag to enable hex display

fig = plt.figure()
ax1 = fig.add_subplot(1,1,1)
'''Serial Value .txt writer'''
filepath_duty_cycle = 'duty_cycle_serial_data.txt'
file_duty_cycle = open(filepath_duty_cycle, 'w', newline='\n')

'''Serial Value .txt writer'''
filepath_rpm = 'rpm_serial_data.txt'
file_rpm = open(filepath_rpm, 'w', newline='\n')
...
class SerialThread(QtCore.QThread):
    ...
    def run(self):  # Run serial reader thread
        ...
        while self.running:
            ...
            if 'LeftValue' in bytes_str(st):
                # left_decoded = data.decode()
                left = bytes_str(st).split(' ')
                left_val = int(left[1])
                # print('Left :', left_val)
            # if 'BottomValue' in data.decode():
            #     bottom_decoded = data.decode()
            #     bottom = bottom_decoded.split(' ')
            #     bottom_val = int(bottom[1])
                # print('Bottom :', bottom_val)

            # if 'RPM: ' in bytes_str(st):
            #     current_rpm = bytes_str(st).split('RPM: ')
            #     file_rpm.writelines(current_rpm)
            # if 'Duty: ' in bytes_str(st):
            #     current_duty_cycle = bytes_str(st).split('Duty: ')
            #     file_duty_cycle.writelines(current_duty_cycle)
            if st:  # Get data from serial port
                self.ser_in(bytes_str(st))  # ..and convert to string
        ...
        if self.ser:  # Close serial port when thread finished
            self.ser.close()
            file_duty_cycle.close()
            read_file_duty = pd.read_csv(r'duty_cycle_serial_data.txt')
            csv_duty_cycle = read_file_duty.to_csv(r'duty_cycle_serial_data.csv', index=None)
            # csv_duty_cycle.to_excel(r'duty_cycle.xlsx', index=None, header=True)
            file_rpm.close()
            read_file_rpm = pd.read_csv(r'rpm_serial_data.txt')
            csv_rpm = read_file_rpm.to_csv(r'rpm_serial_data.csv', index=None)
            # csv_rpm.to_excel(r'rpm.xlsx', index=None, header=True)

            # graph_data = open('rpm_serial_data.txt', 'r').read()
...
if __name__ == "__main__":
    ...
    w.show()
    # ani = animation.FuncAnimation(fig, SerialThread.run, interval=1000)
    # plt.show()
    sys.exit(app.exec_())
